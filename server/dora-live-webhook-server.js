...
const server = http.createServer(async (req, res) => {
  ...
  // ── 7. محاكاة رسالة تجريبية لاختبار شاشة المتصفح مباشرة ──────────────────
  if (pathname === '/api/simulate-incoming' && (method === 'GET' || method === 'POST')) {
    const simulationPresets = [
      {
        text: 'السلام عليكم، كم سعر فحمات كيا سبورتاج 2022 متوفرة بالفرع؟',
        lead: { carModel: 'كيا سبورتاج 2022', interestType: 'فحمات فرامل', city: 'بريدة' }
      },
      {
        text: 'وين موقعكم بالضبط في بريدة؟ وفيه فرع مفتوح العصر؟',
        lead: { carModel: 'هيونداي سوناتا', interestType: 'زيارة فرع', city: 'بريدة' }
      },
      {
        text: 'لو سمحت متوفر شحن لمدينة الرياض لباب البيت؟',
        lead: { carModel: 'جينيسيس G80', interestType: 'شحن وتوصيل', city: 'الرياض' }
      }
    ];

    const preset = simulationPresets[Math.floor(Math.random() * simulationPresets.length)];
    const simId = `sim_${Date.now()}`;
    const senderId = 'sim_user_' + Math.floor(1000 + Math.random() * 9000);

    const testItem = {
      id: simId,
      platform: 'meta_instagram',
      channelType: 'dm',
      senderName: `عميل انستغرام (${senderId.slice(-4)})`,
      senderUsername: `customer_${senderId.slice(-4)}`,
      senderId,
      avatar: `https://ui-avatars.com/api/?name=Customer+${senderId.slice(-4)}&background=C13584&color=fff`,
      text: preset.text,
      timestamp: 'الآن (مباشر 🔴)',
      ...
      intent: 'spare_parts',
      isLive: true,
      leadInfo: preset.lead,
      chatHistory: [
        {
          id: `m_${simId}`,
          sender: `عميل انستغرام (${senderId.slice(-4)})`,
          isPage: false,
          message: preset.text,
          time: new Date().toISOString()
        }
    ...
    // معالجة الرد الآلي بالذكاء الاصطناعي للمحاكاة أيضاً
    if (isAutoPilotEnabled) {
      const match = matchDoraBotTriggers(preset.text);
      if (match) {
        testItem.status = 'replied';
        testItem.reply = match.reply;
        testItem.repliedAt = 'الآن (رد آلي درة بوت 🤖)';
        testItem.chatHistory.push({
          id: `rep_${simId}`,
          sender: 'درة السيارة (بوت ذكي 🤖)',
          isPage: true,
          message: match.reply,
    ...
    broadcastToClients('new_message', testItem);
    return sendJson(res, { success: true, message: 'Simulated incoming message sent to all active tabs', item: testItem });
  }

  // ── 8. محاكاة وارد تيك توك (TikTok Lead & Direct Simulator) ───────────────
  if (pathname === '/api/simulate-tiktok' && (method === 'GET' || method === 'POST')) {
    const tiktokPresets = [
      {
        text: 'السلام عليكم، شفت إعلانكم على تيك توك، بكم طقم فحمات ومساعدات كيا سبورتاج 2021 وهل عندكم شحن للرياض؟',
        channelType: 'dm',
        adTitle: 'إعلان تيك توك: حملة المبيعات 3/8/2026 (قطع كورية أصلية) - ID: tt-camp-001',
        lead: { carModel: 'كيا سبورتاج 2021', interestType: 'فحمات ومساعدات', city: 'الرياض', phone: '0554128930' }
      },
      {
        text: 'طلب تسعيرة عبر نموذج تيك توك الفوري (TikTok Lead Ad): محتاج صدام أمامي وشمعات هيونداي النترا 2022 ديزل',
        channelType: 'lead',
        adTitle: 'نموذج تيك توك الفوري: Traffic 22/7/2026 (زيارات المتجر والسلات) - ID: tt-camp-002',
        lead: { carModel: 'هيونداي النترا 2022 ديزل', interestType: 'صدام أمامي وشمعات', city: 'عنيزة', phone: '0531198450' }
      },
      {
        text: 'شفت مقطع التيك توك حقكم عن صيانة الديزل، بكم طقم تسخين وبواجي ديزل لبونجو 3؟',
        channelType: 'dm',
        adTitle: 'إعلان تيك توك: Community 10/8/2026 (فحمات وبكجات الصيانة) - ID: tt-camp-003',
        lead: { carModel: 'كيا بونجو 3 ديزل', interestType: 'طقم تسخين وبواجي', city: 'بريدة', phone: '0503418821' }
      }
    ];

    const preset = tiktokPresets[Math.floor(Math.random() * tiktokPresets.length)];
    const simId = `tt_live_${Date.now()}`;
    const randDigits = Math.floor(1000 + Math.random() * 9000);
    const senderName = `عميل تيك توك (${randDigits})`;
    ...
    const ttItem = {
      ...
      senderName,
      senderUsername: `tiktok_user_${randDigits}`,
      senderId: `tt_${randDigits}`,
      avatar: `https://ui-avatars.com/api/?name=TikTok+${randDigits}&background=000000&color=00f2fe`,
      text: preset.text,
      timestamp: 'الآن (مباشر تيك توك 🎵)',
      rawTime: new Date().toISOString(),
      status: 'pending',
      intent: 'spare_parts',
      sentiment: 'positive',
    ...
    if (isAutoPilotEnabled) {
      const match = matchDoraBotTriggers(preset.text);
      if (match) {
        ttItem.status = 'replied';
        ttItem.reply = match.reply;
        ttItem.repliedAt = 'الآن (رد آلي درة بوت 🤖)';
        ttItem.chatHistory.push({
          id: `rep_${simId}`,
          sender: 'درة السيارة (بوت ذكي 🤖)',
          isPage: true,
          message: match.reply,
    ...
    recentMessages.unshift(ttItem);
    if (recentMessages.length > 50) recentMessages.pop();

    console.log(`🎵 [TikTok Live Hub] وصول رسالة حية من تيك توك: [${senderName}]: "${ttItem.text}"`);
    broadcastToClients('new_message', ttItem);

    return sendJson(res, {
      success: true,
      message: 'تم إرسال رسالة تيك توك الحية للبث المباشر بنجاح! 🎵',
      item: ttItem
    });
  }

  // ── 9. نقطة استقبال ويب هوك تيك توك الرسمي (TikTok Webhook) ───────────────
